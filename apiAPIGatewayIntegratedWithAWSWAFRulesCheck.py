...
def make_opensearch_connection():
    # get the opensearch variable from the env.yaml file
    env = EnvYAML("./env.yaml")
    host = env['DEV.opensearch_host']
    port = env['DEV.opensearch_port']
    region = env['DEV.opensearch_region']
...
def rules_check(event, context):
        ...
        try:
            ...
            for value in response1['items']:            
                ...
                for stageItem in response2['item']:
                    data = {
                        "errorCode": "",
                        "ruleName": "API Gateway Integrated With AWS WAF",
                        "organisationId": organisationId,
                        "serviceName": "API Gateway",
                        "resourceName": "stageName " + stageItem['stageName'],
                        "ruleID": "AG-007",
                        "ruleRemarks": "should be achieved",
    ...
    if (len(result) > 0):
        try:
            client = make_opensearch_connection()
            index_name = "rules-check-cybergate-dev-index"
            resp = helpers.bulk(client, result, index=index_name)
